...
import net.wellfield.magpie.MagpieFileUtil;
...
public class Device implements HTTPRequestListener, SearchListener
{
	...
	//TODO make dynamic or enable to set
	...
	public void httpRequestRecieved(HTTPRequest httpReq, HTTPSocket sock)
	{
		if (Debug.isOn() == true)
			httpReq.print();
	
		if (httpReq.isGetRequest() == true) {
			httpGetRequestRecieved(httpReq, sock);
			return;
		}
		if (httpReq.isPostRequest() == true) {
			httpPostRequestRecieved(httpReq, sock);
			return;
		}
	...
	private void httpGetRequestRecieved(HTTPRequest httpReq, HTTPSocket sock)
	{
		...
		if (isDescriptionURI(uri) == true) {
		    ...
		    File f = null;
		    if (uri.endsWith(".mpg")) {
		        f = new File("c:/rip/demo.mp4");
		    } else if (uri.endsWith(".mp3")) {
		        f = new File("c:/rip/viva.mp3");
		    } else {
	            httpReq.returnBadRequest();
	            return;
		    }
		    fileByte = MagpieFileUtil.fileToByteArray(f);
		    Debug.message("File " + f.getAbsolutePath() + " exists: " + f.exists());
		}
		else {
			httpReq.returnBadRequest();
			return;
	...
	private void httpPostRequestRecieved(HTTPRequest httpReq, HTTPSocket sock)
	{
		if (httpReq.isSOAPAction() == true) {
			//SOAPRequest soapReq = new SOAPRequest(httpReq);
			soapActionRecieved(httpReq, sock);
			return;
		}
		httpReq.returnBadRequest();
	}
	...
	private void soapActionRecieved(HTTPRequest soapReq, HTTPSocket sock)
	{
		String uri = soapReq.getURI();
		Service ctlService = getServiceByControlURL(uri);
		if (ctlService != null)  {
			ActionRequest crlReq = new ActionRequest(soapReq);
			deviceControlRequestRecieved(crlReq, ctlService, sock);
			return;
		}
	...
	private void deviceControlRequestRecieved(ControlRequest ctlReq, Service service, HTTPSocket sock)
	{
		if (ctlReq.isQueryControl() == true)
			deviceQueryControlRecieved(new QueryRequest(ctlReq), service, sock);
		else
			deviceActionControlRecieved(new ActionRequest(ctlReq), service, sock);
	}
	...
	private void deviceActionControlRecieved(ActionRequest ctlReq, Service service, HTTPSocket sock)
	{
		if (Debug.isOn() == true)
			ctlReq.print();
		...
		if (action.performActionListener(ctlReq, sock) == false)
			invalidActionControlRecieved(ctlReq);
	}

	private void deviceQueryControlRecieved(QueryRequest ctlReq, Service service, HTTPSocket sock)
	{
		if (Debug.isOn() == true)
			ctlReq.print();
